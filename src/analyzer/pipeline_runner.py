...
from analyzer.workflows import (ai_categorization, bank_transaction_analysis,
                                minimal_load)
...
def parse_args():
    ...
    parser.add_argument(
        "--workflow",
        default="bank_transaction_analysis",
        choices=list(WORKFLOW_REGISTRY.keys()),
        help="Workflow to run",
    )
    parser.add_argument(
        "--log-level", default="INFO", help="Logging level (e.g., DEBUG, INFO, WARNING)"
    )
    parser.add_argument(
        "--metadata-dir",
        default=None,
        help="Directory to store metadata (defaults to ~/.metadata/pipelines)",
    )
...
def main():
    ...
    pipeline_metadata = PipelineMetadata(
        pipeline_name=args.workflow, start_time=datetime.now(), end_time=datetime.now()
    )
    ...
    if isinstance(result_df, pd.DataFrame) and result_df.empty:
        ...
        logging.info(
            f"[pipeline_runner] Workflow completed. Output rows: {len(result_df) if isinstance(result_df, pd.DataFrame) else 'N/A'}, total time: {elapsed:.2f} seconds"
        )
