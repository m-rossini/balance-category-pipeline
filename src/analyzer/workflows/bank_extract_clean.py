...
def create_transaction_number(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.reset_index(
        drop=True
    )  # Ensure the index is reset before assigning transaction numbers
    df["TransactionNumber"] = (
        len(df) - df.index
    )  # Generate transaction numbers in reverse order
    return df
...
def bank_extract_clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "SortCode" in df.columns:
        df["SortCode"] = df["SortCode"].astype(str).str.replace("'", "", regex=False)
    ...
    if "CreditAmount" in df.columns:
        ...
        df["CreditAmount"] = pd.to_numeric(df["CreditAmount"], errors="coerce").fillna(
            0
        )
